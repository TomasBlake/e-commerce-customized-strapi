...
module.exports = {
  ...
  async delete(ctx) {
    const { userAbility } = ctx.state;
    const { id, model } = ctx.params;
    await axios.delete(`${process.env.SNIPCARD_MAIN_API_ENDPOINT}/products/${id}`,{
      headers: {
          'content-Type': 'application/json',
          'accept': 'application/json',
          'authorization': `Basic ${Buffer.from(process.env.SNIPCART_SECRETE_API_KEY,'utf8').toString('base64')}`
      }
    });
